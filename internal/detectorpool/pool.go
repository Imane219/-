...
var poolSetting *setting.PoolSettingS

func New(poolSet *setting.PoolSettingS) *DetectorPool {
	poolSetting = poolSet
	return &DetectorPool{}
}
...
func (dp *DetectorPool) GeneID() string {
	...
	go func() {
		timer := time.NewTimer(poolSetting.DetectorExpiredTime)
...
func (dp *DetectorPool) StartCmd(id, runTime string) error {
	...
	dp.resetTimer(id, poolSetting.DetectorExpiredTime)
	return nil
}
